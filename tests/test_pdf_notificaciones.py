...
from models import Factura, MetodoPago, Pedido, Usuario, db
...
def test_generar_pdf_comprobante_valido():
    with app.app_context():
        ...
        pdf_bytes, nombre_archivo, factura = generar_pdf_pedido(pedido, tipo="COMPROBANTE_PENDIENTE")
...
def test_factura_final_bloqueada_si_no_esta_aprobado(client):
    with client:
        ...
        with app.app_context():
            usuario = Usuario.query.filter_by(email="cliente@newrecords.local").first()
            pedido = Pedido.query.filter_by(cliente_id=usuario.id, estado="PENDIENTE").first()
            numero = pedido.numero
...
def test_notificaciones_ejecutan_sin_error():
    with app.app_context():
        usuario = Usuario.query.filter_by(email="cliente@newrecords.local").first()
        pedido = Pedido.query.filter_by(cliente_id=usuario.id).first()

        # En modo dev (sin SMTP real) retornan False limpiamente sin lanzar excepciones
